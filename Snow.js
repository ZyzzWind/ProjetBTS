    ...
    const count = 5000
    let wind = { current: 0, force: 0.1, target: 0.1, min: 0.1, max: 0.1, easing: 0.01 }

    const snow = new ShaderProgram( document.querySelector( '.particles' ), {
      depthTest: false,
      texture: snowflake,
      uniforms: {
        worldSize: { type: 'vec3', value: [ 0, 0, 0 ] },
        gravity: { type: 'float', value: 80 },
        wind:{ type: 'float', value: 1 },
      },
      buffers: {
        size: { size: 1, data: [] },
        rotation: { size: 2, data: [] },
        speed: { size: 3, data: [] },
      },
      vertex: `
